...
| **Utilidad de la segunda API (y las adicionales)** | Ver [sección de APIs externas](#apis-externas) — todas están **integradas al flujo principal** (diagnóstico del chat o registro del cliente), no son llamadas aisladas. |
...
ClickIA integra **cuatro** APIs además de la base de datos, todas en el flujo principal de la plataforma (no llamadas sueltas):
...
4. **OpenCage Geocoding** (dirección → coordenadas, **sí requiere API key**, gratis hasta 2500 peticiones/día) — al registrar un cliente (por sí mismo o por el admin), convierte su dirección de casa en coordenadas reales; el admin ve un enlace "Ver en mapa" en la tabla de usuarios para ubicar exactamente dónde despachar un técnico, en vez de solo tener el texto libre de la dirección.

Clima e ISP se consultan solo cuando hay un problema técnico real identificado (no en preguntas de plan/cuenta); las tres (clima, ISP, geocoding) se degradan sin romper el flujo si fallan, no tienen API key configurada, o no están disponibles, y sus resultados se **guardan en la base de datos** (tablas `diagnostics` y `users`), no solo se usan y se descartan.
...
- [x] Cuatro APIs externas (clima, proveedor/ISP, geocoding) integradas al diagnóstico/registro y persistidas en BD
...
- [x] Despliegue con URL pública (Render) — ver [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md)
- [x] Propuesta de segundo proyecto de IA — ver [docs/PROPUESTA_PROYECTO_IA.md](docs/PROPUESTA_PROYECTO_IA.md)
